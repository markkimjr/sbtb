...
from sbtb.fighter.dependencies import get_boxer_repo, get_boxing_scraper, get_boxer_org_repo, get_rank_repo, \
    get_weight_class_repo, get_boxing_fight_card_scraper
...
@router.get("/update-boxing-fight-cards",
            ...
            response_model=List[RankSchema],
            tags=["fighters"])
async def scrape_and_save_boxing_schedules(scraper: BoxingFightCardScraper = Depends(get_boxing_fight_card_scraper),
                                           fighter_repo: FighterRepo = Depends(get_boxer_repo),
                                           organization_repo: FightingOrganizationRepo = Depends(get_boxer_org_repo),
                                           rank_repo: RankRepo = Depends(get_rank_repo),
                                           weight_class_repo: WeightClassRepo = Depends(get_weight_class_repo)
                                           ) -> Response:
    service = BoxingFightCardService(scraper=scraper, fighter_repo=fighter_repo,
                                     organization_repo=organization_repo,
                                     rank_repo=rank_repo,
                                     weight_class_repo=weight_class_repo)
    updated_ranks: List[RankSchema] = await service.scrape_and_update_boxing_fight_cards()
    return JSONResponse(
        content={"updated_boxing_fight_cards": jsonable_encoder(updated_ranks)},
        status_code=status.HTTP_200_OK,
    )
