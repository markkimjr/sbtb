...
from sbtb.fighter.schemas import RawBoxerSchema, WeightClassRead
...
class BoxingRankScraper(BaseScraper):
    ...
    def parse(self, raw_data: str) -> Dict[str, Dict[str, List[RawBoxerSchema]]]:
        ...
        for i in range(total_sections // 2):
            ...
            weight_class = WeightClassRead(name=division, pounds=pounds)
    ...
    def parse_fighter(self, fighter_div: bs4.element.Tag, idx: int) -> List[RawBoxerSchema]:
        ...
        if name_span.find("br"):
            ...
            primary_champ = RawBoxerSchema(name=primary_name, rank=idx, is_champ=True)

            # remove any text after "(" in the secondary name
            if "(" in secondary_name:
                secondary_name = secondary_name.split("(")[0].strip()
            secondary_champ = RawBoxerSchema(name=secondary_name, rank=0.5, is_champ=True)
            return [primary_champ, secondary_champ]

        name_str = name_span.text
        name = name_str.split("(")[0].strip().lower()
        fighter = RawBoxerSchema(name=name, rank=idx, is_champ=True if idx == 0 else False)
        return [fighter]
...
class BoxingFightCardScraper(BaseScraper):
    ...
    async def request_data(self) -> Optional[str]:
        try:
            ...
            accept_button = wait.until(EC.element_to_be_clickable((By.XPATH, '//button[@aria-label="Accept All"]')))
            accept_button.click()
    ...
    def parse_fight_card(self, fight_card_a_tag: bs4.element.Tag) -> Optional[Dict[str, Any]]:
        ...
        try:
            fight_date_class = (
                "bg-red text-white font-display font-bold text-xs md:text-sm uppercase px-2.5 md:px-4 py-1.5 md:py-2"
            )
            fight_date_div = fight_card_a_tag.find("div", {"class": fight_date_class})
            ...
            # title fighters
            title_fight_div = fight_card_a_tag.find(
                "div", {"class": "flex flex-row gap-2 md:gap-3 px-2.5 md:px-4 pt-1.5 md:pt-2 items-center align-middle"}
            )
            ...
            # location
            meta_p = fight_card_a_tag.find(
                "p", {"class": "text-black text-xs md:text-sm font-semibold leading-5 line-clamp-1"}
            )
