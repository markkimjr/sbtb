...
import { searchMockFighters } from "@/lib/mock-fighters";

export default function FightersPage() {
  const [query, setQuery] = useState("");
  const fighters = useMemo(() => searchMockFighters(query), [query]);
  ...
  return (
    <div className="flex-1 flex flex-col">
      <div className="px-9 pt-7 pb-3 flex items-center justify-between gap-6">
        <div className="flex flex-col gap-1">
          <h1 className="font-display italic text-2xl leading-tight tracking-tight">
            Pick your{" "}
            <span className="not-italic text-[var(--color-persimmon)]">
              fighters.
            </span>
          </h1>
          <p className="text-[13px] opacity-70">
            We ring the{" "}
            <span className="font-display italic text-[14px]">bell</span> when
            they fight next.
          </p>
        </div>
      ...
      <FighterCarousel
        fighters={fighters}
        renderBookmark={(f) => (
          <BookmarkButton fighterId={f.id} fighterName={f.name} />
        )}
      />
